...
import { z } from 'zod'
import { CAMERA_PRESETS } from '@/lib/video/kling/motion-presets'

/**
 * Kling Camera Control API
 * カメラワークプリセットまたは参照動画を使用してカメラ制御を適用
 */
...
async function fetchCropAndUpload(
  ...
  const fileName = `kling-input/${userId}/${Date.now()}-camera-${targetAspectRatio.replace(':', 'x')}.jpg`
...
const cameraPresetIds = CAMERA_PRESETS.map(p => p.id) as [string, ...string[]]

const cameraControlSchema = z.object({
  type: z.enum(['pan', 'tilt', 'zoom', 'roll', 'truck', 'dolly']),
  direction: z.enum(['left', 'right', 'up', 'down', 'in', 'out', 'cw', 'ccw']).optional(),
...
const cameraSchema = z.object({
  ...
  // Camera options
  cameraPresetId: z.enum(cameraPresetIds).optional(),
  cameraReferenceVideoUrl: z.string().url().optional(),
  cameraControls: z.array(cameraControlSchema).max(5).optional(),
  // Standard params
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validationResult = cameraSchema.safeParse(body)
    ...
    try {
      ...
      console.error('[Kling Camera] Image crop error:', cropError)
      return NextResponse.json(
    ...
    if (videoError || !videoData) {
      console.error('[Kling Camera] DB Error:', videoError)
      return NextResponse.json(
    ...
    return NextResponse.json({
      ...
      jobId: job.id,
      message: 'カメラ制御動画生成を開始しました。完了まで1〜3分かかります。',
    })
  } catch (error) {
    console.error('[Kling Camera] Error:', error)
    return NextResponse.json(
